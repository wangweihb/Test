...
var publishPro = function() {
	...
	var title = $("#title").val();
	var pic = $("#pic").val();
	if(isNullOrEmpty(title) || isNullOrEmpty(contentHtml) || isNullOrEmpty(pic)){
		alert("所有内容都必须填写，请您仔细检查！");
		return false;
	}else{
		$("#welfatePublishForm").submit();
	}
}